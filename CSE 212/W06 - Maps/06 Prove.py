...
class Translator:
    ...
    def translate(self, from_word):
        ...
        """
        # Check to see id the word is in the self.words dictionary and translate it. If not return "???"
...
def is_anagram(word1, word2):
    ...
    # Note: This can be done shorter by writing a function to convert
    # each word to a dictionary.
    word1_dict = {}
    word2_dict = {}
    ...
    for letter in word1:
        ...
        letter = letter.lower()
        # Ensure letter is not a whitespace.
        if letter != " ":
...
class Maze:
    ...
    def move_left(self):
        ...
        # Check if location can be moved to.
        if allowed[0]:
            self.curr_x -= 1
        else:
...
def earthquake_daily_summary():
    ...
    # Iterate through 'features', displaying earthquake data for each location.
