...
class LinkedList:
    ...
    def insert_tail(self, value):
        ...
        # If the list is empty, then point both head and tail
        # to the new node
        if self.head is None:  # could also check self.tail
            self.head = new_node
            self.tail = new_node
        # If the list is not empty, then only self.tail will be
        # affected.
        else:
            new_node.prev = self.tail  # Connect the new node to the previous tail
            self.tail.next = new_node  # Connect the previous tail to the new node
            self.tail = new_node       # Udpate the tail to point to the new node
    ...
    def remove_tail(self):
        ...
        """
        # If the list has only one item it, then set head and tail 
        # to None resulting in an empty list.  This condition will also
        # cover an empty list.  Its okay to set to None again.
        if self.head == self.tail:
            self.head = None
    ...
    def remove(self, value):
        ...
        while curr is not None:
            if curr.data == value:
                ...
                if curr == self.head:
                    self.remove_head()
                # If the lcoation of 'value' is at the end of the list,
                # then we can call remove_tail
                elif curr == self.tail:
                    self.remove_tail()
                # For any other location of 'value', need change the links to 
                # bypass the node containing 'value'.
                else:
    ...
    def replace(self, old_value, new_value):
        ...
        to 'new_value'.
        """
        # Search for all nodes that match 'old_value' by starting at the
        # head of the list.
        curr = self.head
        while curr is not None:
            # If a match is found, then change the value to 'new_value'
